 ...
 * If MongoDB is connected via setMongoDatabase(), it uses MongoDB. Else, falls back to in-memory cache.
 * TTL is determined dynamically based on the lyrics status.
 */

const { config } = require('./config.js');
const { validateLyricsMatch } = require('./match-validator.js');
const { normalizeForMatch } = require('./track-metadata.js');

// In-memory cache fallback map
const memoryCache = new Map();

// Dynamic MongoDB instance
let customDb = null;

/**
 * Injects a MongoDB database instance dynamically.
 * @param {object} dbInstance - MongoDB Database connection instance (e.g., client.db('name'))
 */
function setMongoDatabase(dbInstance) {
  customDb = dbInstance;
}
...
function getCollection() {
  if (customDb && typeof customDb.collection === 'function') {
    try {
      const col = customDb.collection('lyrics_cache');
...
const getCacheKey = lyricsCacheKey;
 ...
 */
async function getCachedLyrics(track) {
 ...
 */
async function setCachedLyrics(track, lyricsData) {
  const key = lyricsCacheKey(track);
  if (!key) return;
 ...
 */
async function clearLyricsCacheForTrack(track) {
  const key = lyricsCacheKey(track);
  if (!key) return false;
 ...
 */
async function getLyricsCacheInfo(track) {
  const key = lyricsCacheKey(track);
  if (!key) return null;
